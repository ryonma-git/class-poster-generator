...
2. 実機カメラ撮影の動作確認・調整（学校iPad等）
   ...
   - **ピンチズーム**（`CameraModel.setZoom`）の感度・上限を実機で調整。

3. **【中優先】クロップ枠の位置を任意で調整できる詳細設定**（ユーザー要望 2026-05-30）
   - 現状: 枠は横中心固定、Y位置は枠の高さから自動算出。
   - 標準UIは触らず、設定画面に「詳細設定」セクションを追加し、そこから
     枠の縦位置（top）・横位置（left）を crop_adjuster のように調整できるように。
   - 普段は不要、撮影位置が偏る場合に追い込み用。
   - 実装案: AppState に `guideTopFrac`/`guideLeftPctFromCenter` を追加し、
     `CropMath.defaultGuideRect` がそれを使う。SetupView に開閉式の詳細セクション。

4. （任意）撮影済み枚数の自動保存・再開（アプリ終了しても途中から続けられる）。
5. （任意）名簿(名前)対応・一覧シート出力など。
